...
* [0007-MADR-docs-completion-audit.md](0007-MADR-docs-completion-audit.md) —
  ...
  “looks solid”).
* `docs/reports/0060-REPORT-drag-and-drop-engine-feasibility.md` — A–E shipped; H17 is the live residual.
* `docs/reports/0057-REPORT-file-view-engine-assessment.md` — L13.
...
| 2026-08-15 | Phase 12 completed: L17 (Settings discloses that Keyboard Mappings and Forget Host save immediately), L18 (Connection / Add Existing submit on Enter and name the first invalid field under the dimmed button), L19 (Stash empty-state points at the context-bar "Stash Changes" CTA), L20 (Worktrees overview walks with ↑/↓ via the same `stepSelection` as Stash). Skipped per plan: L3, L4, L6, L11–L15, L21, L22. |
| 2026-08-15 | Post-completion audit of the whole backlog against `master`: analyzer clean, full suite green (3106 passing, the 2 `live-forge` skips untouched), and every phase's landmark symbol still present after the later navigator/help-book refactors. One L18 gap fixed — `AddExistingRepoSheet._submit` did not honor `_canSubmit` (the connection form's does), so Enter in the git-dir field opened a scoped repo *unscoped* while the Open button was dimmed; guarded, with a focused test. Status moved `proposed` → `accepted`. |
| 2026-08-15 | L20 deselect parity: the Worktrees overview now selects (and focuses) the row a right-click opens its menu over — as Stash's card menu does — and gains the canonical `DeselectOnEmptyClick` wrapper, so a click on empty list space clears the selection like every other selectable panel. |
