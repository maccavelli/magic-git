...
| 0025 | [Host process economy](0025-MADR-unaccounted-host-side-work.md) | `accepted` | [plan](0025-PLAN-unaccounted-host-side-work.md) | `in-progress`; phases 1-5, 7, 9 executed; re-measured 2026-09-04 (123->76 processes, idle now free). C2/D2/D3/F1 settled without code; new findings C4 (degraded-poll path = 53% of observed load; diagnosed and fixed under 0026) and C5 (orphans kept alive by their successor's heartbeat, invisible to the sweep — open). Phase 12 still unapproved |
...
| 0027 | [Watcher reclamation cannot reclaim](0027-MADR-watcher-reclamation-cannot-reclaim.md) | `accepted` | [plan](0027-PLAN-watcher-reclamation-cannot-reclaim.md) | `proposed`; 0025 C5 decided. **The connect sweep has never been able to kill anything** — it records a shell pid (`comm=sh`) and signals only `inotifywait`/`fswatch`, and the check is `/proc`-only so it is doubly dead on macOS hosts. Four further defects compound it; remedy is per-instance lease/registry files with `ps`-verified identity. 5 phases, awaiting approval |
