...
| 0026 | [Degraded-watch poll diagnosis](0026-MADR-degraded-watch-poll-diagnosis.md) | `proposed` | [plan](0026-PLAN-degraded-watch-poll-diagnosis.md) | `proposed`; diagnosis only — instruments `watchLifecycle`'s mode transitions to settle why it polls while host watchers live (0025 C4). Leading hypothesis H1: `start()` has no re-entrancy guard and can double-reserve a watcher slot. **No fix under this number** |
