...
| 0025 | [Host process economy](0025-MADR-unaccounted-host-side-work.md) | `accepted` | [plan](0025-PLAN-unaccounted-host-side-work.md) | `in-progress`; phases 1-5, 7, 9 executed; re-measured 2026-09-04 (123->76 processes, idle now free). C2/D2/D3/F1 settled without code; new finding C4 (degraded-poll path = 53% of observed load) outranks the unbuilt D1. Phase 12 still unapproved |
| 0026 | [Degraded-watch poll diagnosis](0026-MADR-degraded-watch-poll-diagnosis.md) | `accepted` (amended 0026.1) | [plan](0026-PLAN-degraded-watch-poll-diagnosis.md) | `in-progress`; **H1 CONFIRMED by test** (`Expected: <2> / Actual: <1>` — two sources armed, one torn down) and fixed by serialising `start()` (deviation (b) took the fix into scope). Phases 1-3, 5 executed; **Phase 4 post-fix capture outstanding**. H2/H3 open, not refuted |
