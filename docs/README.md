...
| 0027 | [Watcher reclamation cannot reclaim](0027-MADR-watcher-reclamation-cannot-reclaim.md) | `proposed` | — | `proposed`; 0025 C5 decided. **The connect sweep has never been able to kill anything** — it records a shell pid (`comm=sh`) and signals only `inotifywait`/`fswatch`, verified live. Three further defects compound it; remedy is per-instance lease/registry files with cmdline-verified identity |
