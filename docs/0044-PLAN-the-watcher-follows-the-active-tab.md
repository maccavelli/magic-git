...
#### Still owed — needs a running build and a person

The installed bundle is not the running one: a census of the host shows **zero**
watcher processes whose script contains `mg-watch: armed`, so the app in memory
is still the pre-fix build (`open` on an already-running app only focuses it).
Until it is restarted, these cannot be checked and are not claimed:

* **the arm's end-to-end cost across tab switches** (acceptance criterion 6) —
  the host-side component is measured above, but the client-side timing from
  `watch()` to the `armed` transition is what the criterion names;
* **the cross-session refusal** (4.4) — the check that matters most, because
  this change must not be describable as having disabled the exclusion it
  speeds up: a second session arming the same repository must produce one honest
  `heldByAnother` naming the incumbent token, and leave no `.hb` behind;
* **the registry census** (4.2) with the new build live.
