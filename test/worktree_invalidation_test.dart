...
void main() {
  test(
    'a build churning ignored files costs nothing, and the diff still shows',
    () async {
      ...
      expect(
        ...
        ['spinner', 'diff'],
        reason:
            'the diff must load once and then stay: the pane spun forever '
      ...
      expect(
        ...
        0,
        reason:
            'nor cost a `git status` — over SSH that is a round trip a second',
      );
      ...
      expect(
        ...
        diffsBefore,
        reason:
            "another file's edit must not refetch this file's diff — the "
