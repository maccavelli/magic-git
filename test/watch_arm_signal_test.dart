...
//
// Elapsed time is FAKE time (MADR 0045 phase 6): a clock that only moves when a
// timer is due measures exactly whether an arm waited on one, which is the
// question, where a stopwatch also measured the machine's load.
...
void main() {
  ...
  /// Arms [handle], lets the arm commit either way, then tears down.
  ///
  ...
  ArmOutcome arm(FakeAsync async, FakeWatcherHandle handle) {
    ...
    while (!settled() && async.elapsed - start < const Duration(seconds: 10)) {
      async.elapse(const Duration(milliseconds: 5));
    }
    final outcome = (
      elapsed: async.elapsed - start,
  ...
  test('an arm settles on the marker, not on the ceiling', () {
    fakeAsync((async) {
      final handle = FakeWatcherHandle.armed();
      final outcome = arm(async, handle);

      // The bound IS the assertion. 250 ms is the constant this change
      // deletes, so anything at or above it means the race is not racing —
      // the arm fell through to a timer, exactly as it did before.
      expect(
        outcome.elapsed,
  ...
  test('a silent host falls through to the ceiling and still arms', () {
    fakeAsync((async) {
      // Neither signal. This is the case the old flat wait handled by
      // accident, and the reason the ceiling is kept: degrade slowly, never
      // wrongly.
      final outcome = arm(async, FakeWatcherHandle.silentHost());
      ...
      expect(
        ...
        isFalse,
        reason:
            'a host that says nothing is treated as armed, as it was before',
      );
  ...
  test('a lock refusal is still a refusal, and still names its incumbent', () {
    fakeAsync((async) {
      final outcome = arm(
        async,
  ...
  test('the refusal releases the lease it stamped', () {
    fakeAsync((async) {
      final outcome = arm(
        async,
        FakeWatcherHandle.refused(
  ...
  test('only the marker settles an arm — chatter does not', () {
    fakeAsync((async) {
  ...
  test('the arm subscribes to stderr exactly once', () {
    fakeAsync((async) {
  ...
  test('a refused arm subscribes to stderr exactly once too', () {
    fakeAsync((async) {
  ...
  test('a marker arriving late does not re-settle a settled arm', () {
    fakeAsync((async) {
      final handle = FakeWatcherHandle.armed();
      final executor = _ScriptedExecutor(handle);
      final service = RemoteWatchService(
      ...
      // A duplicated line, or a script that re-announced. Completing an
      // already completed completer throws, which would kill the arm from
      // inside its own stderr listener.
