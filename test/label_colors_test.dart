...
void main() {
  group('tryParseLabelColor', () {
    test('parses a full RRGGBB hex with #', () {
      ...
      expect(c, isNotNull);
      expect((c!.r * 255).round(), 0xFF);
      expect((c.g * 255).round(), 0x88);
      expect((c.b * 255).round(), 0x00);
    });

    test('parses hex without #', () {
      final c = tryParseLabelColor('aabbcc');
      expect(c, isNotNull);
      expect((c!.r * 255).round(), 0xAA);
      expect((c.g * 255).round(), 0xBB);
