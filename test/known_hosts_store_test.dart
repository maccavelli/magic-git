...
void main() {
  ...
  test(
    ...
    () async {
      ...
      expect(
        (await store.lookup('example.com', 22))!.fingerprint,
        'SHA256:new',
      );
    },
  );
  ...
  test('malformed entries are skipped; valid ones still load', () async {
    ...
    expect(
      (await store.lookup('good.example.com', 22))!.fingerprint,
      'SHA256:ok',
    );
  });

  test('concurrent remembers for different hosts are serialized, not lost to a '
  ...
  test('an IPv6-literal host with colons does not collide with another '
      'host:port pair', () async {
    final store = KnownHostsStore();
    await store.remember(
      '::1',
      22,
    ...
    expect((await store.lookup('fe80::1', 2222))!.fingerprint, 'SHA256:other');
  });

  test(
    'list enumerates every remembered host, sorted by host then port',
    () async {
      final store = KnownHostsStore();
      await store.remember(
        'b.example.com',
      ...
      expect(list.map((r) => '${r.host}:${r.port}').toList(), [
        'a.example.com:22',
        'a.example.com:2222',
        'b.example.com:22',
      ]);
