...
void main() {
  ...
  Future<(_Trace, _FakeHandle)> subscribe() async {
    final t = _Trace();
    glab.traceStream('/repo', 900).listen(
      t.events.add,
  ...
  test('#13: a delivered log with a non-zero trace exit is NOT an error',
      () async {
  ...
  test('#3: a zero-output clean close emits one empty terminal tick, no error',
      () async {
    final (t, h) = await subscribe();
  ...
  test('a trace that produced no stdout but wrote stderr surfaces an error',
      () async {
    final (t, h) = await subscribe();
  ...
  test('#1: an error on the stderr substream surfaces as a stream error',
      () async {
    final (t, h) = await subscribe();
