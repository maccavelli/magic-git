...
void main() {
  test(
    'operation anchors survive interleaving and report their first line',
    () {
      final container = ProviderContainer();
      addTearDown(container.dispose);
      final log = container.read(outputLogProvider.notifier);
      const first = OperationId('first');
      const second = OperationId('second');
      log.logResult(
        'git fetch',
      ...
      log.logInfo('unrelated');
      log.logError('git push', 'failed', operationId: second);

      expect(log.firstIndexForOperation(first), 0);
      expect(log.firstIndexForOperation(second), 4);
    },
  );

  group('OutputLogState', () {
    test(
      'default-constructed state has visible=false, no lines, revision 0',
      () {
    ...
    test('copyWith overrides selected fields', () {
      ...
      expect(
        state
            .copyWith(lines: [const OutputLine('hi', OutputLineKind.info)])
            .lines,
        hasLength(1),
      );
    });
    ...
    test('withLines advances revision', () {
      const state = OutputLogState();
      final updated = state.withLines([
        const OutputLine('x', OutputLineKind.info),
      ]);
  ...
  group('OutputLogNotifier', () {
    ...
    test(
      'logResult formats a command with stdout, stderr, and success line',
      () {
        final container = ProviderContainer();
        addTearDown(container.dispose);
        final log = container.read(outputLogProvider.notifier);
        ...
        log.logResult(
          'git status',
          const SSHCommandResult(exitCode: 0, stdout: 'clean\n', stderr: ''),
        );
  ...
  group('OutputStreamSession', () {
    test('append with plain text emits command header and finalized lines', () {
      ...
      expect(
        lines,
        hasLength(3),
      ); // command + 2 finalized (no trailing partial)
    ...
    test('CR progress frames are collapsed to the last frame', () {
      ...
      expect(
        lines,
        hasLength(2),
      ); // command + 1 finalized (no trailing partial)
