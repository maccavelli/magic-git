...
class _CommitComposerState extends ConsumerState<CommitComposer> {
            ...
            children: [
              ...
              Row(
                children: [
                  ...
                  // The only moving pixel on this surface while the commit and
                  // push run. Without it the sheet was static for the whole
                  // wait, which is what made a working app read as a hung one.
  ...
  Widget _statusText(
    ...
    // While committing, say what is happening. This used to keep showing
    // "Review, Edit, or Accept" for the whole commit+push — instructing the
    // user to press a button the same state had disabled, with no spinner and
    // no other feedback anywhere on the surface (0023 P1).
    final text = controller.committing
        ? (controller.localCommitLanded
    ...
    return Text(
      ...
      overflow: TextOverflow.ellipsis,
      style: MacosTheme.of(context).typography.caption1.copyWith(
        color: controller.error == null || controller.committing
